...
> 单文件版中国小说转漫画 SKIIS。用户只需要上传本文件 + 小说章节原文/链接 + 基础角色图 + 上一期 `handoff.json`，即可直接生成本章漫画页 P01-P10 与新的 `handoff.json`。
...
```text
输入一章小说，直接输出中国漫画成品页 P01-P10，并生成 handoff.json；下一章继续上传本文件 + 基础角色图 + 上一期 handoff + 新章节原文/链接，即可接着生产。
```
...
系统必须自动新建空白 `handoff.json`，并把基础角色图作为最高优先级角色锚点。
...
系统必须读取上一期 `handoff.json`，继承角色外观、道具状态、场景状态和未解决钩子，然后直接生成下一章 P01-P10 与新的 `handoff.json`。
